...
public static class FirebirdTools
{
        ...
        string user, string password, bool appendDbPath, params string[] extraArgs)
    {
        ...
        catch (Exception ex) { result.Error = $"Error ejecutando {tool}: {ex.Message}"; }
        return result;
    }
    ...
    {
        // Genera el backup con gbak; la restauración la hace el usuario con el motor Firebird 4.0 (genera ODS 13).
